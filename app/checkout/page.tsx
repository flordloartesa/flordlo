...
// 👈 NOVO: Importamos o cliente do Sanity para ler as configurações
...
export default function CheckoutPage() {
  ...
  // 👈 NOVO: Estado para guardar as configurações de portes vindas do Sanity
  ...
  // 👈 NOVO: Vai ao Sanity buscar as configurações assim que a página abre
  useEffect(() => {
  ...
  // ==========================================
  // 🧮 1. LÓGICA DE PREÇOS E DESCONTOS
  // ==========================================
  ...
  // ==========================================
  // 📦 2. LÓGICA DE PESOS E PORTES DE ENVIO (SINCRONIZADA COM SANITY)
  // ==========================================
  const totalWeight = cart.reduce((acc: number, item: any) => acc + (Number(item.weight || 0) * (item.quantity || 1)), 0);
  
  // Numa loja de flores, todos os produtos são físicos, logo a morada é sempre obrigatória!
  ...
  if (hasPhysicalProduct) {
    if (shippingSettings) {
      const custoBase = Number(shippingSettings.shippingCost || 4.80);
      const portesGratisLimite = Number(shippingSettings.freeShippingThreshold || 0);
      const pesoBaseLimite = Number(shippingSettings.baseWeightLimit || 2);
      const custoExtraKg = Number(shippingSettings.extraPricePerKg || 1.50);

      if (portesGratisLimite > 0 && subtotal >= portesGratisLimite) {
        shippingCost = 0; // 🎉 Portes Grátis!
      } else {
        shippingCost = custoBase;
        // ⚖️ Regra do Peso Extra
        if (totalWeight > pesoBaseLimite) {
          const pesoExtra = totalWeight - pesoBaseLimite;
          const kgExtraCobrados = Math.ceil(pesoExtra);
          shippingCost += (kgExtraCobrados * custoExtraKg);
        }
      }
    } else {
      // Fallback visual enquanto o Sanity carrega
      shippingCost = 4.80; 
    }
  }
  ...
  const handleStripeCheckout = async () => {
    ...
    const itemsForStripe = cart.map(item => ({
      ...item,
      _id: item._id.replace("drafts.", "").substring(0, 36),
      name: item.title, 
      price: item.price,
    ...
    try {
      const response = await fetch('/api/create-stripe-session', {
        ...
        body: JSON.stringify({
          ...
          total: totalFinal,
          shippingCost: shippingCost, // 👈 Enviamos o custo lido para backup do backend
        }),
      });
          ...
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-2xl p-8 md:p-12 max-w-lg w-full shadow-2xl text-center animate-in zoom-in duration-300">
              ...
              <p className="text-slate-500 mb-8 leading-relaxed">
                Enviámos um e-mail para <span className="font-bold text-slate-800">{formData.email}</span> com os dados para transferência bancária.
              </p>

              <div className="flex flex-col gap-3">
                <button 
                  onClick={() => router.push("/")} 
                  className="w-full py-4 bg-[#9d6b73] text-white rounded-sm font-bold tracking-widest uppercase flex items-center justify-center gap-3 transition-colors hover:bg-[#85585f]"
                >
        ...
        {errorModal.show && (
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-2xl p-8 md:p-10 max-w-sm w-full shadow-2xl text-center animate-in zoom-in duration-300">
              ...
              <button 
                onClick={() => setErrorModal({ show: false, message: "" })}
                className="w-full py-4 bg-slate-900 text-white rounded-sm font-bold tracking-widest uppercase transition-colors hover:bg-black"
              >
                Tentar Novamente
              </button>
            </div>
              ...
              <div className="bg-white p-6 md:p-8 rounded-xl shadow-sm border border-slate-100">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <input 
                    ...
                    className={`w-full p-4 border rounded-md outline-none focus:border-[#9d6b73] transition-all text-sm ${session ? 'bg-slate-100 border-transparent text-slate-500' : 'bg-slate-50 border-transparent focus:bg-white'}`} 
                    placeholder="E-mail *" 
            ...
            <div className="lg:col-span-1">
              <div className="bg-white p-6 md:p-8 rounded-xl shadow-md border border-slate-100 lg:sticky lg:top-24">
                ...
                <div className="border-t border-slate-100 pt-4 mb-8 space-y-3">
                  ...
                  {/* 👇 NOVA ZONA DOS PORTES COM CONDIÇÃO PARA "GRÁTIS" */}
                  <div className="flex justify-between items-center text-sm text-slate-500">
