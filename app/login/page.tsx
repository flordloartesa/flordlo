      ...
      <div className="max-w-md w-full bg-white p-10 rounded-[32px] shadow-2xl border border-slate-100">
        ...
        <div className="text-center mb-8 flex flex-col items-center">
          <img 
            src={logoUrl} 
            alt="Flordlo Logo" 
            className="w-12 h-12 rounded-[100px] object-cover mb-4 shadow-sm"
          />
          <h2 className="text-3xl font-extrabold text-slate-900 tracking-tight">Flor.d.ló</h2>
          <p className="text-slate-500 mt-2">
            {mode === "login" ? "Bem-vinda/o de volta!" : "Cria a tua conta gratuita"}
          </p>
        </div>
        ...
        <div className="flex bg-slate-100 p-1 rounded-2xl mb-8">
          <button 
            ...
            className={`flex-1 py-2 text-sm font-bold rounded-xl transition-all ${mode === "login" ? "bg-white text-[#C47F8D] shadow-sm" : "text-slate-500"}`}
          >
            Entrar
          </button>
          <button 
            onClick={() => { setMode("register"); setMessage({type:"", text:""}); }}
            className={`flex-1 py-2 text-sm font-bold rounded-xl transition-all ${mode === "register" ? "bg-white text-[#C47F8D] shadow-sm" : "text-slate-500"}`}
          >
            Registar
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {mode === "register" && (
            <div className="relative">
              ...
              <input 
                ...
                className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none focus:ring-2 focus:ring-[#C47F8D]/20 text-black shadow-sm"
          ...
          <button 
            ...
            className="w-full py-4 bg-[#D88A96] text-white font-bold rounded-2xl hover:bg-slate-800 transition-all flex items-center justify-center gap-2 shadow-lg disabled:bg-slate-400 transform hover:-translate-y-0.5 active:translate-y-0"
          >
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          >
            {loading === 'google' ? <Loader2 className="animate-spin" size={18}/> : <Chrome size={18} className="text-[#C47F8D]" />} Google
          </button>
          ...
          <button 
            ...
            className="flex items-center justify-center gap-2 py-3 border border-slate-100 rounded-2xl hover:bg-slate-50 transition-all text-sm font-bold text-slate-700 shadow-sm disabled:opacity-50"
          >
            {loading === "magic" ? <Loader2 className="animate-spin" size={18} /> : <Mail size={18} className="text-[#C47F8D]" />} Magic Link
          </button>
        </div>

        <div className="text-center mt-8">
          <Link href="/" className="text-xs text-slate-400 hover:text-[#C47F8D] transition-colors font-medium">Voltar ao site</Link>
        </div>
      </div>
    </div>
  );
